...
# Sua string de conexão do Neon DB
...
def init_db():
    try:
        ...
        cur.close()
        conn.close()
        print("Banco de dados verificado/conectado com sucesso!")
...
def login():
    ...
    if username == 'geoagreste' and password == 'provisoria1':
...
def get_licitacoes():
    ...
    licitacoes = [row[0] for row in rows]
            ...
            <form id="bidForm" onsubmit="saveBid(event)">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" id="idPncp" required placeholder="Ex: 13891130000103-1-000135/2026">
                    </div>
                    <div class="form-group">
                        <label>Fonte</label>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="local" placeholder="Ex: Vera Cruz/BA">
                    </div>
                    <div class="form-group">
                        <label>Órgão</label>
                        <input type="text" id="orgao" required placeholder="Ex: MUNICIPIO DE VERA CRUZ">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="datetime-local" id="dataDoc" required>
                    </div>
                    ...
                    <div class="form-group full-width">
                        ...
                        <textarea id="objeto" required placeholder="Descreva o objeto da licitação..."></textarea>
                    </div>
                </div>
    ...
    <script>
        ...
        function renderMapMarkers() {
            ...
            ativas.forEach(lic => {
                ...
                L.marker(coordenadas).addTo(markersGroup).bindPopup(`
                    <strong>${lic.orgao}</strong><br>${(lic.objeto || '').substring(0, 50)}...<br>
        ...
        function renderAlerts() {
            ...
            ativas.forEach(lic => {
                ...
                if (diferencaHoras < 0) {
                    statusAlert = 'urgent';
                    textAlert = `⚠️ <b>ATRASADA!</b> O prazo de documento para ${lic.orgao} expirou.`;
                } else if (diferencaHoras <= 48) {
                    statusAlert = 'warning';
                    textAlert = `⏳ <b>URGENTE:</b> Docs para ${lic.orgao} vencem em ${Math.ceil(diferencaHoras)}h!`;
                } else {
                    statusAlert = 'info';
                    textAlert = `ℹ️ Docs de ${lic.orgao} no prazo (Vence ${formatarData(lic.dataDoc)}).`;
                }
        ...
        function renderDashboard() {
            ...
            document.getElementById('dashboard-stats').innerHTML = `
                <div class="card card-encontrada"><h3>Encontrada</h3><div class="value">${qtdEncontrada}</div></div>
                <div class="card card-analise"><h3>Em Análise</h3><div class="value">${qtdAnalise}</div></div>
                <div class="card card-doc"><h3>Documentação</h3><div class="value">${qtdDoc}</div></div>
                <div class="card card-disputa"><h3>Disputa / Sessão</h3><div class="value">${qtdDisputa}</div></div>
            `;
                ...
                container.innerHTML += `
                    <div class="kanban-column" ondrop="drop(event, '${col.id}')" ondragover="allowDrop(event)">
                        ...
                        <div class="kanban-cards">
                            ${itens.map(lic => {
                                const isExpired = new Date(lic.dataFim) < new Date() ? 'expired' : '';
                                return `
                                <div class="task-card" draggable="true" ondragstart="drag(event, '${lic.uid}')" onclick="abrirEdicao('${lic.uid}')">
                                    <div class="task-id">${lic.idPncp}</div>
                                    <div class="task-title">${(lic.objeto || '').substring(0, 50)}...</div>
                                    <div class="task-org"><b>${lic.orgao}</b> <span>${lic.local}</span></div>
        ...
        function renderArquivadas() {
            ...
            container.innerHTML = itens.map(lic => `
                <div class="card" style="display:flex; justify-content: space-between; align-items: center; border-left: 4px solid #64748b; margin-bottom: 10px;">
                    <div>
                        <div class="task-id">${lic.idPncp}</div>
                        <strong>${lic.orgao}</strong> - ${(lic.objeto||'').substring(0, 60)}...
        ...
        function abrirEdicao(uid) {
            ...
            document.getElementById('modalTitle').innerText = 'Editar Licitação: ' + lic.idPncp;
            document.getElementById('bidModal').classList.add('active');
        }
