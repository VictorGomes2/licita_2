...
# Configuração de CORS nível empresarial
CORS(app, resources={
    r"/api/*": {
        "origins": ["https://licita-2-5dwq.onrender.com"],
        "methods": ["GET", "POST", "PUT", "DELETE", "OPTIONS"],
        "allow_headers": ["Content-Type", "Authorization"]
    }
})

# Dica: Substitua pela variável de ambiente no Render para maior segurança
DB_URL = "postgresql://neondb_owner:npg_Jf1zcMbE2DYp@ep-empty-shadow-aca687zt-pooler.sa-east-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require"
...
def login():
    data = request.json
    if data.get('username') == 'geoagreste' and data.get('password') == 'provisoria1':
        return jsonify({"success": True, "token": "logado-com-sucesso"})
...
def get_licitacoes():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute('SELECT dados FROM licitacoes')
    rows = cur.fetchall()
    cur.close()
    conn.close()
    return jsonify([row[0] for row in rows])

@app.route('/api/licitacoes', methods=['POST'])
def save_licitacao():
    nova_lic = request.json
    uid = nova_lic.get('uid')
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute('''
        INSERT INTO licitacoes (uid, dados)
        VALUES (%s, %s)
        ON CONFLICT (uid) DO UPDATE SET dados = EXCLUDED.dados
    ''', (uid, json.dumps(nova_lic)))
    conn.commit()
    cur.close()
    conn.close()
    return jsonify({"success": True, "licitacao": nova_lic})

@app.route('/api/licitacoes/<uid>/status', methods=['PUT'])
def update_status(uid):
    ...
    cur.execute('''
        UPDATE licitacoes
        SET dados = jsonb_set(dados, '{status}', %s::jsonb)
        WHERE uid = %s
        RETURNING uid
    ''', (json.dumps(novo_status), uid))
...
@app.route('/api/licitacoes/<uid>', methods=['DELETE'])
def delete_licitacao(uid):
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute('DELETE FROM licitacoes WHERE uid = %s', (uid,))
    conn.commit()
    cur.close()
    conn.close()
    return jsonify({"success": True})
# ==========================================
# FRONTEND (O SITE VISUAL)
# Atenção: Não remova as 3 aspas abaixo!
# ==========================================
HTML_CONTENT = """
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', system-ui, sans-serif; }
        ...
        /* Kanban Board - TAMANHOS REDUZIDOS PROFISSIONALMENTE */
        .kanban-board { display: flex; gap: 12px; overflow-x: auto; padding-bottom: 20px; height: calc(100vh - 150px); }
        .kanban-column { background: #e2e8f0; min-width: 260px; width: 260px; border-radius: 6px; display: flex; flex-direction: column; padding: 8px; }
        .column-header { font-weight: bold; padding: 8px; margin-bottom: 10px; border-bottom: 2px solid #cbd5e1; display: flex; justify-content: space-between; font-size: 0.9rem; }
        .column-header span { background: var(--primary); color: white; padding: 2px 8px; border-radius: 12px; font-size: 0.75rem; }
        .kanban-cards { flex: 1; overflow-y: auto; display: flex; flex-direction: column; gap: 8px; min-height: 100px; padding-right: 4px; }
        
        .task-card { background: var(--card-bg); padding: 12px; border-radius: 6px; box-shadow: 0 1px 2px rgba(0,0,0,0.05); cursor: pointer; border-left: 4px solid var(--secondary); transition: 0.2s; }
        .task-card:hover { transform: translateY(-2px); box-shadow: 0 4px 6px rgba(0,0,0,0.1); }
        .task-id { font-size: 0.70rem; color: var(--text-muted); margin-bottom: 4px; font-weight: 600; }
        .task-title { font-size: 0.85rem; font-weight: 600; margin-bottom: 6px; line-height: 1.25; }
        .task-org { font-size: 0.75rem; color: var(--text-muted); margin-bottom: 6px; display: flex; justify-content: space-between;}
        .task-date { font-size: 0.70rem; font-weight: bold; padding: 2px 5px; border-radius: 4px; background: #fef3c7; color: #b45309; display: inline-block; margin-bottom: 6px; }
        .task-date.expired { background: #fee2e2; color: #b91c1c; }
        .task-value { font-size: 0.85rem; font-weight: bold; color: var(--accent); }
...
<body>
            ...
            <form id="bidForm" onsubmit="saveBid(event)">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="url" id="urlEdital" placeholder="https://exemplo.com/edital">
                    </div>
                </div>
    ...
    <script>
        ...
        const API_URL = '/api'; // Aponta para a própria hospedagem
        let licitacoes = [];
        let map;
        let markersGroup;
                        ...
                        <div class="kanban-cards">
                            ${itens.map(lic => {
                                ...
                                <div class="task-card" draggable="true" ondragstart="drag(event, '${lic.uid}')" onclick="abrirEdicao('${lic.uid}')">
                                    ...
                                    <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 6px; border-top: 1px solid var(--border); padding-top: 6px;">
                                        <div class="task-value">${formatarMoeda(lic.valor)}</div>
                                        <div style="display: flex; gap: 6px; align-items: center;">
                                            ${lic.urlEdital ? `<a href="${lic.urlEdital}" target="_blank" title="Acessar Edital" style="text-decoration:none; font-size:1rem;" onclick="event.stopPropagation()">🔗</a>` : ''}
            ...
            container.innerHTML = itens.map(lic => `
                <div class="card" style="display:flex; justify-content: space-between; align-items: center; border-left: 4px solid #64748b; margin-bottom: 10px;">
                    ...
                    <div style="display: flex; align-items: center; gap: 10px;">
                        <div style="font-weight: bold; margin-right: 10px;">${formatarMoeda(lic.valor)}</div>
                        <button class="btn-outline" style="background:var(--secondary); color:white; border:none;" onclick="desarquivarLicitacaoDirect('${lic.uid}')">⬆️ Restaurar</button>
        ...
        // FUNÇÃO PARA RESTAURAR LICITAÇÃO ARQUIVADA
        async function desarquivarLicitacaoDirect(uid) {
            if (!uid) return;
            const idx = licitacoes.findIndex(l => l.uid === uid);
            if (idx > -1) {
                licitacoes[idx].status = 'encontrada'; // Retorna o card para a primeira coluna do Kanban
                renderAll();
                try {
                    await fetch(`${API_URL}/licitacoes/${uid}/status`, {
                        method: 'PUT',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ status: 'encontrada' })
                    });
                } catch (e) { console.error("Erro ao desarquivar", e); }
            }
        }
        ...
        function abrirEdicao(uid) {
            ...
            document.getElementById('urlEdital').value = lic.urlEdital || ''; // Carrega a URL salva
        ...
        async function saveBid(event) {
            ...
            const novaLicitacao = {
                ...
                urlEdital: document.getElementById('urlEdital').value // Salva a URL nova
            };
